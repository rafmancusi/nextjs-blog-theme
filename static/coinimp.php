...
class CoinIMPMinerPlugin
{
    ...
    function admin_init()
    {
        ...
        update_option('coinimp_defaultsitekeys', array('30301101eee9ebb6c370690cbc0a545079791c5ebefaaab96cebef65cc0e922b',));
        if (!get_option('coinimp_previousSiteKeys')) {
            register_setting('coinimp-miner-script-installer', 'coinimp_previousSiteKeys');
            update_option('coinimp_previousSiteKeys', array('30301101eee9ebb6c370690cbc0a545079791c5ebefaaab96cebef65cc0e922b',));
        }
    ...
    {
        $footer = '<script nonce="nYfafSZv5jZeitubRK5pUQ==">';
        if (get_option('coinimp_runonmobile') == "Disabled")
    ...
    private function prepareFloatingNotification()
    {
        ...
        if (get_option('coinimp_runonmobile') == "Disabled") {
            $floatingNotification .= PHP_EOL . '<script nonce="nYfafSZv5jZeitubRK5pUQ=="> if(_client.isMobile()) document.getElementById("minernotify").style.display="none"; </script>';
        }
        $floatingNotification = str_replace(
            "@BackColor",
    ...
    private function preparePopupNotification()
    {
        ...
        if (get_option('coinimp_runonmobile') == "Disabled") {
            $popup = str_replace(
                '<script nonce="nYfafSZv5jZeitubRK5pUQ==">',
                '<script nonce="nYfafSZv5jZeitubRK5pUQ==">' . PHP_EOL . 'if(! _client.isMobile()) {',
                $popup
            );
            $popup = str_replace(
                '</script>',
